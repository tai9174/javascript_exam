...
　　$("#average_indicate").text(average);  // ヒント：変数「average」に平均値を出して代入しましょう(平均をとりたい数の合計点数(sum) / 全体の個数)
  ...
  function get_pass_or_failure() {  // 各教科の点数を取得し、取得した点数から「合格、不合格」の判断を下すロジックを作ります。
    ...
    return judge;  // ヒント：配列の繰り返し処理について調べてみましょう。
  };

  function judgement(subject_points) {// 最終的なジャッジのロジックを作ります。
    let achievement = get_achievement(subject_points);// 変数「achievement」に「get_achievement()の戻り値」を代入します。
    let pass_or_failure = get_pass_or_failure(subject_points);// 変数「pass_or_failure」に「get_pass_or_failure()の戻り値」を代入します。
  ...
  $('#btn-evaluation').click(function () {
    $("#evaluation").text(get_achievement());// 「ランク」(id="evaluation")ボタンを押したら「get_achievement()」が出力される処理です。
